...
def _replace_table_reference(sql: str, table_candidates: list[str]) -> tuple[str, str | None]:
    ...
    replacement = f"iceberg_catalog.gold.{preferred_table}"
    pattern = re.compile(
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        if normalized_ref.startswith("iceberg_catalog.gold.") and normalized_ref.rsplit(".", 1)[-1] == preferred_table:
            return match.group(0)
        changed = True
    ...
    if changed:
        return corrected, f"Replaced table reference with iceberg_catalog.gold.{preferred_table}."
    return sql, None
...
def _replace_disallowed_catalog(sql: str, table_candidates: list[str]) -> tuple[str, str | None]:
    ...
    if corrected != sql:
        return corrected, "Replaced disallowed catalog/schema with iceberg_catalog.gold."
    return sql, None
...
Rules:
- Use only iceberg_catalog.gold tables listed above.
