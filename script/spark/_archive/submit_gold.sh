#!/usr/bin/env bash
# Submit the consolidated GoldJob manually from the host via the Airflow container.
...
echo "==> Submitting GoldJob from container ${SPARK_SUBMIT_CONTAINER} ..."
echo "==> GOLD_RUN_MODE=${GOLD_RUN_MODE}"
echo "==> GOLD_REFRESH_MODE=${GOLD_REFRESH_MODE}"
...
docker exec \
  ...
  /opt/project/code/spark/gold_job.py
