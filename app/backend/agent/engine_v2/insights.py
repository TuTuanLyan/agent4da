"""Insight generator (ported from code/api/insight_generator.py).

`generate_insight` is a pure rule-based answer builder. `generate_llm_insight`
asks the LLM for a short Vietnamese analysis and ALWAYS falls back to the
rule-based answer when the LLM is unavailable or returns something unsafe.
"""
...
from . import llm
from .config import MAX_LLM_ROWS
...
def _with_source(result: dict[str, Any], source: str, error: str | None = None) -> dict[str, Any]:
    output = {**result, "insight_source": source, "llm_insight_used": source == "llm"}
    if error:
...
def _metric_overview_answer(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if parts:
        return {"answer": f"Trong dữ liệu hiện tại có {', '.join(parts)}.", "insights": parts}
...
def _ranking_answer(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if dimension_key and metric_key:
        ...
        return {
            ...
            "insights": [f"Top result: {dimension_key}={dimension_value}, {metric_key}={metric_value}."],
        }
...
def _revenue_answer(rows: list[dict[str, Any]], warnings: list[str]) -> dict[str, Any]:
    ...
    if value is None:
        return {"answer": "Không tìm thấy trường doanh thu rõ ràng trong kết quả hiện tại.", "insights": []}
...
def generate_insight(
    *,
    ...
    if not rows:
        return {"answer": "Không có dữ liệu phù hợp với câu hỏi trong dữ liệu hiện tại.", "insights": []}
    ...
    return {"answer": f"Truy vấn trả về {len(rows)} dòng trong dữ liệu hiện tại.", "insights": []}
...
def generate_llm_insight(
    *,
    ...
    if intent not in LLM_INSIGHT_INTENTS or not rows or row_count == 0:
    ...
    try:
        ...
        parsed["answer"], parsed["insights"] = _enforce_revenue_caveat(parsed["answer"], parsed["insights"], warnings)
