"""SQL guard (ported from code/api/sql_guard.py), catalog standardized to `iceberg`.

Allows only read-only SELECT / WITH / SHOW TABLES / DESCRIBE against
`iceberg.gold.*` plus the project semantic metadata tables. Generic metadata
queries are rewritten away from `SHOW TABLES` and `information_schema` because
the Iceberg JDBC catalog uses schema-version V0. Rejects DDL/DML,
multi-statement SQL, comma joins, and unqualified/foreign tables. Auto-appends
LIMIT to non-aggregate detail queries.
"""
...
DIM_TABLES = {"dim_time", "dim_product", "dim_user", "dim_session"}
FACT_TABLES = {"fact_events", "fact_sales"}
...
def _uses_fact_table(sql: str) -> bool:
    ...
    fact_refs = {f"{GOLD_CATALOG}.{GOLD_SCHEMA}.{table}".lower() for table in FACT_TABLES}
